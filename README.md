...
6. **volontiranje** - ponuda i potražnja 
7. **pomoć u radu** - npr. branje jabuka
8. **osnovne škole** - npr. suveniri
9. **srednje škole** - npr. proizvodi iz njihovih radionica ili povezivanje obrtničkih škola s gospodarstvom
10. **visoko školstvo** - npr. povezivanje s gospodarstvom
11. **hobiji** - povezivanje ljudi oko hobija
12. **zabava** - povezivanje oko raznih oblika zabave, npr. restorani, zabavni parkovi
13. **sport** - nije ponuda poslova za sport nego aktivnosti vezane uz sportske klubove, skupljanje članova, projekti, skupljanje donacija
14. **turizam** - nije ponuda poslova za turizam, nego turistička ponuda u određenom mjestu uključujući i lokalne prehrambene proizvode
...
Više osoba zajedno mogu napraviti veće i složenije stvari, zbog čega je jako korisno ako web aplikacija omogućava lagano udruživanje osoba oko zajedničkih ciljeva.

Udruživanje počinje slanjem pozivnice drugoj osobi.

Svaka osoba može istovremeno biti udružena na više različitih mjesta s različitim osobama oko različitih ciljeva.
...
U svim scenarijima udruživanja uvijek će jedna osoba biti glavna osoba i ona će biti vlasnik podataka.

Fizičke osobe mogu pozivati druge fizičke ili pravne osobe na udruživanje.

## Organiziranje dijeljenih podataka u grupe i podgrupe

Potreba za dijeljenjem podataka je prva stvar koja će se dogoditi kad se više osoba udruži oko zajedničkog cilja. Što više udruženih osoba, to će količina dijeljenih podataka biti veća. Kako bi se učinkovito snalazilo u većim količinama podataka, u ovoj web aplikaciji koriste se grupe i podgrupe za organiziranje i učinkovito upravljanje podacima.

Grupe i podgrupe su uvijek obavezne, čak i kad se radi o manjim količinama podataka, ali korist od njih dolazi do izražaja tek kod složenijih scenarija koji uključuju mnogo udruženih osoba s većim količinama podataka u dužem vremenskom periodu.

Unutar grupa nalaziti će se podgrupe, a unutar podgrupa nalaziti će se aktivnosti.
...
Dvije su razine pristupa, prva je čitaj/piši, druga je samo čitaj (kod ove razine nije moguće ažurirati podatke u bazi podataka).
...
# Druga mogućnost web aplikacije je popunjavanje profila radnih vještina, znanja i sposobnosti

Profili radnih vještina su polustrukturirani kroz tri opcionalne razine detaljnosti:
1. **Prva razina** je općenita.
2. **Druga razina** je srednje detaljno.
3. **Treća razina** je detaljno.
...
Ime, prezime i id kandidata fizičkih osoba se anonimizira.

Anonimizacija se provodi šifriranjem AES algoritmom na razini aktivnosti, to znači da će isti kandidat imati različito anonimizirano ime, prezime i id na različitim aktivnostima.
...
Fizička osoba upravlja svojom anonimizacijom i može odlučiti hoće li točno određenom klijentu isključiti anonimizaciju.
...
2. **bilješka o vrsti profila** - odnosi se na jedan od profila, npr. zapošljavanje, volontiranje, prekvalifikaciju, ...
...
4. istovremeno 2. i 3.
5. **bilješka o osobi** - to je osoba koja nije anonimizirana nego je poznato njezino ime, prezime i id, može se kombinirati sa 2. i/ili 3.
...
10. **bilješka o kandidatu** 
     - u slučaju anonimizacije nije poznat kandidat, pa je ovo način da se pridruži bilješka toj nepoznatoj osobi
...
11. **bilješka o svojstvu** 
     - svojstvo je radna vještina, znanje ili sposobnost koja pripada kandidatu
...
S odabranim kandidatima moguće je razmjenjivati poruke koje mogu dovesti do novih suradnji.
...
# Dizajn je prilagođen uređajima s ekranima različitih veličina
...
Ovo je glavni ekran tražilice radnih vještina preko kojeg je moguće pratiti mnoge indikatore vezane uz ocjenjivanje i odabir kandidata.
...
Ono što se može primjetiti prema nazivu kandidata je anonimizacija naziva kandidata.
...
U slučaju rada s više pridruženih članova, jedni drugima će vidjeti napisane bilješke prema pravima pristupa postavljenim na grupe i podgrupe aktivnosti.
